...
- **Letting agents transition Briefs to a terminal state.** Acceptance (`done`) and rejection (`wontfix`) are human-only in v1. Terminal Events must be attributed to a human Actor. Three human paths reach terminals: (1) the PR-merge webhook (ADR 007) — the merger's GitHub identity is recorded as the `done` Actor; (2) the PR-close-without-merge webhook — the closer is the `wontfix` Actor; (3) human UI confirm (`done`) or reject (`wontfix`) via `major-confirm-qa` / `major-reject-brief`. None of these are agent transitions. When a terminal state is reached and the Brief has `source_issue_*` populated, a resolution comment is posted and the source issue is closed per ADR 011 — this is also a human-attributed action (best-effort, does not block the transition).
