// functions/major-reaper/index.ts
//
// POST /major-reaper
//   body: {} (none)
//   200:  { cancelledRuns: number, itemsReturned: number }
//
...
//      - UPDATE work_items SET status='ready-for-agent' (clean retry)
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    return jsonResponse({
      ...
      itemsReturned: row?.items_returned ?? 0,
    });
  } catch (err) {
