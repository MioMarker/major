...
supabase/
  config.toml          per-function gateway settings (verify_jwt = false
                       for webhook + reaper)
  migrations/          DDL + RPCs — applied via `supabase db push`
    20260509000000_initial_schema.sql
    20260509000001_rpc_functions.sql   (Postgres RPCs called via supabase.rpc())
  functions/
...
Apply the migrations (initial schema + RPCs both live under
`supabase/migrations/` and ship together):

```
SUPABASE_DB_PASSWORD=<password> npx -y supabase db push --dry-run   # preview
SUPABASE_DB_PASSWORD=<password> npx -y supabase db push
```
...
`supabase/functions/deno.json` defines a top-level type-check task:
...
(`supabase/migrations/20260509000001_rpc_functions.sql`), called from the
edge layer via `supabase.rpc()`.
