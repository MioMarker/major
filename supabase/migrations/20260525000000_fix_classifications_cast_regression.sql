-- 20260525000000_fix_classifications_cast_regression.sql
--
-- Restores the classifications-cast fix that 20260511000004 introduced and
-- that 20260512000001 + 20260513000000 silently reverted.
--
-- The create-brief branch cast classifications via (jsonb)::text[]::text[],
-- which Postgres rejects with "cannot cast type jsonb to text[]". This made
-- EVERY create-brief op (auto-triage and human apply) fail once 20260513000000
-- reached a database. Use jsonb_array_elements_text instead — identical to the
-- expected_paths handling in the same function and to the 20260511000004 fix.
--
-- This migration keeps ALL of 20260513000000's behavior (issue #80 / ADR 010
-- inbound source-issue attribution: source_issue_* preferred from the parent
-- session's trigger_payload, and author_actor = 'agent:triage-tachikoma' for
-- GitHub-seeded sessions) and changes ONLY the classifications cast (line ~92
-- of 20260513000000) plus restores the `authenticated` execute grant that
-- 20260511000004 had and 20260513000000 dropped (needed for the human
-- Pending-QA apply path).
--
-- Depends on:
--   20260513000000_apply_change_set_source_issue_attribution.sql  (prior body)
  ...
  loop
    ...
    if v_op.operation_type = 'create-brief' then
      insert into major.briefs (
        ...
        'ready-for-triage',
        -- FIX (regression of 20260511000004): jsonb arrays cannot be cast
        -- directly to text[]; unnest via jsonb_array_elements_text.
