-- 20260511000002_apply_change_set_auto_brief_ref.sql
--
-- Two additive enhancements to apply_change_set, required for auto-triage:
--
-- 1. __auto__ placeholder.  When a payload's `brief_id` field is the literal
--    string "__auto__", the RPC substitutes the id of the Brief most recently
--    created in the same change set (captured from a preceding create-brief op).
--    This lets auto-triage emit a single atomic change set:
--
--      Op 0: create-brief               → v_new_brief_id = X
--      Op 1: set-ready-state            payload.brief_id = "__auto__" → resolved to X
--
--    Backwards-compatible: payloads without "__auto__" are unaffected.
--
-- 2. Source-tracing fields on create-brief.  If the payload includes any of
--    `source_session_id`, `source_issue_repo`, or `source_issue_number`, they
--    are written to the briefs row for traceability back to the originating
--    triage session and GitHub issue.  All three are nullable; existing payloads
