...
- State lives in `major.*` schema on the shared dev Supabase project (`nuihvxluxdpdjgkvtdih.supabase.co`). One environment in v1; no separate Major prod.
- Drives **HealthBite** (`~/Projects/healthbite`) and **Healix** (`~/Projects/healix`); opens PRs into their `dev` branches.
- This skill is for AI assistants and humans working *on* Major's code. Tachikomas (Major's own runtime agents) load prompts from `runner/prompts/<role>.md`; they don't read this skill.
...
1. **DB is authoritative.** Don't read lifecycle state from GitHub, files, or env. The `major.*` schema is truth.
2. **Content vs Metadata.** Lifecycle code reads structured columns (`status`, `expected_paths`, `queue_rank`). It does not parse Work Item Content (Markdown PRDs).
3. **Propose / apply separation.** Only Triage Change Operations and Run Finalization Transactions mutate Items. Don't add side-effect writes from chat handlers, webhooks, or background jobs.
4. **Single Active Run Rule.** Never bypass `idx_runs_single_active`. Two simultaneous Runs on one Item is a bug.
...
7. **Prompt versioning.** Bump `*_PROMPT_VERSION` in `runner/prompts/versions.ts` to `<role>@YYYY-MM-DD` whenever a Tachikoma prompt changes.
8. **PRs target `dev`.** `main` is release; `dev → main` is an explicit human act outside the Item lifecycle.
9. **No secrets in code or prompts.** Service-role keys, OpenAI keys, GitHub tokens — env vars only.
10. **Acceptance is human-only.** Agents never transition Items to `done`; the `done` Event must be attributed to a human Actor.

## Where things live

| Area | Path | Notes |
|---|---|---|
| Schema | `supabase/migrations/` | Append-only, `YYYYMMDDHHMMSS_<name>.sql` |
...
| Gateway | `supabase/config.toml` | `verify_jwt = false` for all major-* (Runner uses `sb_secret_*`, not a JWT) |
| UI | `ui/` | Next.js App Router; Supabase auth; deployed to Vercel by manual CLI |
| Runner image | `runner/` | `Dockerfile`, `main.ts`, `tachikoma.ts`, `prompts/` |
| Tachikoma prompts | `runner/prompts/{implementer,reviewer,planner,triage,repair}.md` | Each phase = fresh `claude` subprocess |
...
- User wants to **operate** Major (deploy a function, fire a runner, unstick a Run, sweep orphans) → `docs/runbook.md` + `docs/failure-modes.md`.
...
- User wants to **trigger work through** Major (file an issue, prepare a PRD, draft Items) → Triage Sessions surface in the UI, or call `major-create-triage-session` directly.
...
- **Runner ≠ Tachikoma.** Runner Instance = the long-lived Docker container with heartbeat + lease. Tachikoma = ephemeral `claude` subprocess inside, one per phase per Run. Don't conflate when reasoning about lifecycle or telemetry.
...
Major's lifecycle model is derived from RelyMD's Foundry (`~/Projects/platform/common/docs/foundry/`). Naming, primitives, and state machine are deliberately close to Foundry's. See `docs/adr/001-major-derived-from-foundry.md` for the inheritance and deliberate cuts.
