...
import { ClaimItemRequest } from "../_shared/schemas/claim-item.ts";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.error("[MajorClaimItem]", error);
...
**Every Event write requires an idempotency key.** Per `SPEC.md`, the key is `(work_item_id, event_type, source_actor, source_delivery_id)`.
...
await client.from("events").insert({
  ...
  source_actor: "runner",
  source_actor_id: runner_instance_id,
  idempotency_key: `${work_item_id}:run-started:${runner_instance_id}:${run_id}`,
  payload,
});
```
...
`major-claim-item` performs:

1. `UPDATE major.work_items SET status='agent-running' WHERE id=$1 AND status='ready-for-agent'` — atomic claim.
...
1. `UPDATE major.runs SET outcome='<terminal>', finalized_at=now() WHERE id=$1 AND outcome='running'` — only the holding Runner can finalize.
2. `INSERT INTO major.work_item_artifacts ...` — produced artifacts.
3. `INSERT INTO major.verification_results ...` — verification outcomes.
4. `UPDATE major.work_items SET status='<next>'` — transition.
5. `INSERT INTO major.events (event_type='run-ended', ...)` with idempotency key.

All atomic. If the Runner's lease has expired between heartbeat and finalize, step 1 affects 0 rows and the whole transaction fails — the Runner aborts, having already lost the claim to the Reaper.
...
- Never log full request bodies if they may contain user content. Log shape and identifiers instead.
