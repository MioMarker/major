...
**Branch:** `major/brief-60`.

Tasks:

1. **Heartbeat-loss abort:** `callHeartbeat` currently discards the response (`main.ts:130-135`). Inspect `renewedRun`; if `false`, set a module-level `leaseLostFlag` and abort the active Run at the next safe boundary (between phases, between CI poll iterations). Closes failure mode #3 + finding F-13. **IMPLEMENTED** — `leaseLostFlag` added to module state; `callHeartbeat` inspects response; `abortOnLeaseLost` helper calls `callFinalizeRun(cancelled, lease-expired)` and is checked after planner, after implementer, and after CI poll. `leaseLostFlag` is also checked in `waitForCI` to skip out early.
2. **Sandbox cleanup:** `cleanupSandbox` (`main.ts:487`) only removes the repo dir. Also remove `/work/.major/` (transcripts, item.json, implementer-output.json, telemetry.jsonl) per `sandbox-discipline.md`. Closes finding F-14. **IMPLEMENTED** — `cleanupSandbox` now removes `brief.json`, `plan.md`, `implementer-output.json`, `telemetry.jsonl`, and any `*.transcript.txt` files from `/work/.major/`.
3. **Token leak:** stop baking `GITHUB_TOKEN` into the clone URL (`main.ts:452`). Use `git -c http.extraheader="Authorization: Bearer $TOKEN"` or a credential helper so the token doesn't land in `.git/config`. Closes finding F-15. **IMPLEMENTED** — clone URL is now `https://github.com/<repo>.git` with token passed via `-c http.extraheader`; `gh auth git-credential` is configured as the git credential helper for subsequent fetch/push calls from the Tachikoma subprocess.
4. **PR-already-exists handling:** when the Tachikoma re-runs against a Brief whose branch already has a PR, the `gh pr create` call fails. Either surface a structured signal in the implementer prompt's output schema (`pr_already_exists: true, pr_url, pr_number`) and have the Shell treat that as success, or have the Shell pre-check via `gh pr list --head major/brief-<id>` before invoking the Tachikoma. **IMPLEMENTED** — `findExistingPr` helper added; `executeRun` runs a pre-check before the implementer phase and if an open PR is found, adopts it (synthetic implementer verification) and skips the planner + implementer.
5. **Auto Triage dispatcher (stub-class, not full):** add a `runTriage` entry that the main loop calls when an `auto_triage_requests` row in `requested` status matches a Brief that's also `ready-for-triage`. Spawns a `purpose='triage'` Run via the same claim path. Out of scope for full triage Tachikoma prompt — that's its own work item — but the dispatcher should exist so the request rows aren't inert (finding F-20). **IMPLEMENTED (stub)** — `autoTriageLoop`, `runAutoTriage`, and supporting REST helpers added; loop is started alongside `mainLoop` and `triageSessionLoop`. Change-set application is deferred pending triage-session integration (noted in code comment).
6. **`__dirname` confirmation:** verify `runner/tachikoma.ts:108` still works in whatever module system the Dockerfile compiles to. If ESM, switch to `import.meta.url` resolution. **CONFIRMED** — `tsconfig.json` sets `"module": "commonjs"`, so `__dirname` is correct. A clarifying comment has been added to `tachikoma.ts`.
