...
5. Events: `Issues`, `Pull requests`, `Check runs`, `Pushes`. (`Issues` drives the inbound `major:triage` label → Triage Session path per ADR 010; the others handle PR merges, check-run verification, and push receipts.)
6. Active: yes.
