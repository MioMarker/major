...
2. `/work/.major/queue.json` — current state of `ready-for-triage`, `ready-for-agent`, and `agent-running` Items, sorted by `queue_rank`. Use this to decide rank placement; don't pick a rank that conflicts.
...
If `contentMd` is materially incomplete (no Acceptance Criteria, no Scope Boundaries, ambiguous goal), your output is a `transition-work-item → needs-info` operation **plus** an `add-content-revision` whose body contains the **Missing Information Request**: a markdown section listing exactly what's missing and how to clarify. Do not pretend you have enough info.

### 2. If you have enough, decide:

#### Classifications

One or more of `bug-fix | feature | refactor | docs | parent | epic`. Use `parent` only if the Item is purely a coordinator (no code changes itself). Use `epic` for top-of-tree containers.
...
Almost always `git-change` for engineering work. `triage-change-set` is reserved for meta-Items (e.g. "rerank these 8 Items") that we don't currently fan out.
...
Pick an integer that places this Item in the right priority tier. Look at `queue.json` to avoid collisions:
...
If `contentMd` references other Items by `Major-item: <id>`, propose `add-relationship` ops:

- `parent-child` for decomposition (this Item is a child of an `epic` or `parent`).
- `blocks` if this Item must complete before another can start.
...
- `transition-work-item → ready-for-agent` if the Item now has Ready-for-Agent Content (acceptance criteria + scope boundaries) AND structured metadata (classifications + expectedPaths + expectedArtifactType + baseBranch + queue_rank).
- `transition-work-item → needs-info` if you decided info is missing in step 1.
...
The path-blocker rule runs at the orchestrator on `transition-work-item → ready-for-agent`. If it intersects, the Change Set is queued for human apply (UI surfaces a `needs-human-apply` badge). Don't try to dodge this.
...
If you're declining to triage (not enough signal, ambiguous, out of scope for automation), set `ok: false` and `operations: []` and include `decline_reason`. The orchestrator marks the Auto Triage Request as `failed` and the Item stays at `ready-for-triage`.
...
- **You do not Auto-Triage another Item as a side effect.** Your scope is the one Item Major handed you in `item.json`. Refer to other Items only via `add-relationship` proposals.
- **You do not create new Items.** `create-item` ops are reserved for Triage Sessions (the human-driven path), not Auto Triage Runs.
